...
var WriteModule = (function() {
  ...
  function init() {
    ...
    $("#search-category-form").on("click", "#click-category", function(e) {
      var searchStr = $(e.delegateTarget)
        .find("input")
        .val();
      ...
      $.ajax(BASE_URL + "/api/Category?search-category-string=" + searchStr, {
        ...
        for (var i = 0; i < categoriesAll.length; i++) {
          ...
          for (var j = 0; j < categoriesAll.length; j++) {
            if (
              i !== j &&
              categoriesAll[i].parent_idx === categoriesAll[j].category_id
            ) {
              nameSet
                .add(categoriesAll[j].p_c_name)
                .add(categoriesAll[j].category_name);
              idSet
                .add(categoriesAll[j].p_c_id)
                .add(categoriesAll[j].category_id);
            }
          }
          nameSet
            .add(categoriesAll[i].p_c_name)
            .add(categoriesAll[i].category_name);
      ...
      $.ajax(BASE_URL + "/api/Category?search-category-string=" + searchStr, {
        ...
        for (var i = 0; i < categories.length; i++) {
          ...
          for (var j = 0; j < categories.length; j++) {
            if (
              i !== j &&
              categories[i].parent_idx === categories[j].category_id
            ) {
              nameSet
                .add(categories[j].p_c_name)
                .add(categories[j].category_name);
  ...
  function selectCategory() {
    ...
    $selectedCategory
      .find("h3")
      .data("cid-set", $checkedCategory.data("cid-set"));
    modalHide();
  }
  return {
    init
